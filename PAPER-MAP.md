...
- **Planned formal-methods preprint, working slug *"An Admissibility Calculus: Authorization, Safety Bridges, and Value Decay"*** — bridge artifact + certify. Standalone preprint **outside the Δt Zenodo numbering** (target venue: arXiv cs.LO at minimum; AI-safety venue possible if framing supports). The Lean stack *is* the contribution; the preprint is exposition of finished work, not new construction. **Status: formalized YES** (`lake build` green; one irreducible theorem family — trajectory triple + no-lift over two distinct witnesses); **paper-ready: scaffold-pending** (title decided; abstract, claim list, theorem targets not yet drafted). Full topology and decision provenance at `~/git/papers/working/calculus-paper-spine-2026-05-28.md`.
