...
**Status: HISTORICAL / NON-BINDING / SNAPSHOT.** The Lean specimen described
here was retired during the v13 custody migration; its last public bytes remain
available at tag `v12.0.0`. This note is preserved only as an audit record. The
specimen was migrated on 2026-06-18 from the playground `wired` workspace
(frozen commit `d26311e`, tag `playground-nqjudgments-freeze-d26311e`).
...
Historical build command at v12: `lake build LeanProofs.Scratch.NQJudgments`.
The retired files were `LeanProofs/Scratch/NQJudgments.lean` plus
`NQJudgments/{Basic,NoFreeLift,Standing}.lean`; none exists in the live v13 tree.
