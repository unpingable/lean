...
through the v16 Governed Transition Boundaries release.

Generated 2026-04-03 after static topology and persistence model results.

**Current custody and v16 status:** v13 established the stable/evidence
...
and its intentional multi-root ownerships; v15 and v16 add public-evidence
surfaces without widening any stable root. See
...
### 26. GT-4A — exact governed-transport public target candidate

| Field | Value |
|-------|-------|
| **Location** | Candidate stable root `LeanProofs/GovernedTransport.lean` with ten leaves; separate evidence root `LeanProofs/GovernedTransportEvidence.lean` with `Hostile`, `CompositionHostile`, and the exact Identity hostile split. Target receipts: [`docs/GT4A-TARGET-INTAKE-RECEIPT_2026-07-20.md`](docs/GT4A-TARGET-INTAKE-RECEIPT_2026-07-20.md), [`docs/GT4A-PUBLIC-COMPATIBILITY-RECEIPT_2026-07-20.md`](docs/GT4A-PUBLIC-COMPATIBILITY-RECEIPT_2026-07-20.md). |
...
### 28. v16 — Governed Transition Boundaries

| Field | Value |
|-------|-------|
| **Location** | Evidence roots `LeanProofs/GovernedTransitionBoundaries.lean` (core; `GovernedTransitionBoundaries/Core.lean`) and `LeanProofs/GovernedTransitionBoundariesEvidence.lean` (six evidence modules plus `Qualification.lean`). Release inventory: [`docs/V16-RELEASE-LEDGER.md`](docs/V16-RELEASE-LEDGER.md); scope: [`docs/V16-PUBLIC-INDEX.md`](docs/V16-PUBLIC-INDEX.md). |
| **Claim** | `ExplicitlyFactorsThrough view target` — one total decoder correct for every source — composes, implies the public `ViewSemantics.Determines` fibre-constancy relation, is blocked by a target-distinguishing collision, and is not restored by a carrier derived solely by deterministic postprocessing of an insufficient view. In one declared seven-coordinate language over 1,024 sources and 128 duplicate-free selections, `internalMinimum` is the unique least selection determining the selected five-component target, exactly two masks determine it, the declared six-field carrier factors that target, and no declared selection factors the modeled hidden relation or the combined six-target result. Five separately scoped witnesses instantiate the negative direction. |
| **Bucket** | Public evidence (generic factorization core + fixed-table dependency calculation + five bounded witnesses) |
| **Status** | **SOUND** — 29 receipts replayed fail-closed by `scripts/check-governed-transition-boundaries-footprint.sh`: 16 axiom-free (including all four core receipts), 7 exactly `[propext]`, 6 exactly `[propext, Quot.sound]`, zero `Classical.choice`, zero `sorryAx`. The ten sources are byte-pinned by `scripts/check-governed-transition-boundaries-crossing.sh` against public SHA-256, public Git blob, and extracted-body SHA-256. Both gates run in CI. |
| **Scope custody** | `PUBLIC-EVIDENCE` under two new registered targets. No stable root, stable-root import list, or `scripts/stable-surfaces.tsv` entry changes; no existing public module depends on the surface. `LeanProofs.lean` imports both roots for regression coverage only, which is not a promotion. The generic statements are standard function-factorization and view-determinacy facts; the finite result is an exhaustive dependency calculation in a declared table and coordinate language. The contribution is the mechanically checked synthesis. |
| **Nonclaims / fix** | The converse of the factorization result is explicitly not claimed: fibre constancy does not construct decoders for arbitrary types. Each witness is bounded to its own fixture — no general authorization theory, temporal validity, expiration, revocation, certificate lifecycle, general amalgamation, CSP consistency, schedulability, causality, proof of occurrence, hyperproperty, physical truth, attestation correctness, authentication, or causal identification. No new generic factorization theorem, Hennessy–Milner characterization, statistical minimal sufficiency, universal six-way independence, universal transition-relative semantics, canonical global carrier, target-independent least representation, arbitrary-carrier minimality, cross-surface composition, runtime conformance, or novelty/priority claim. "Transition-relative computation" is a bounded program label, not an operational semantics. No fix required inside this boundary. |
...
| SOUND | 20 | No change; some need cross-referencing (incl. #16 WDC 2.0 normalization, #17 v11 paid recomposition, #18 judgment orientation, #19–#25 the v14 rungs 1–7, and #28 the v16 governed transition boundaries) |
