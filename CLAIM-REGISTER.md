...
| **Nonclaims / fix** | The converse of the factorization result is explicitly not claimed: fibre constancy does not construct decoders for arbitrary types. Each witness is bounded to its own fixture, and "transition-relative computation" is a bounded program label, not an operational semantics. No novelty or priority is claimed, and no runtime conformance follows from the Lean proofs. Complete non-claim ledger: [`docs/V16-PUBLIC-INDEX.md`](docs/V16-PUBLIC-INDEX.md#explicitly-unearned); per-witness fences: [`docs/V16-GOVERNED-TRANSITION-BOUNDARIES.md`](docs/V16-GOVERNED-TRANSITION-BOUNDARIES.md). No fix required inside this boundary. |
