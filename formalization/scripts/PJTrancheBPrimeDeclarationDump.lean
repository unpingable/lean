...
/-
  Compiled declaration census for the provisional PJ Tranche B-prime
  candidate.  Output is canonical JSON Lines using the same normalization as
  the ratified PJ-A census.
-/
...
namespace PJTrancheBPrimeDeclarationDump
...
def canonicalLevelParamName (index : Nat) : Name :=
  Name.mkNum `_pj_tranche_b_prime_universe index
  ...
  for moduleName in selectedModules do
    ...
    for (name, info) in declarations do
      ...
      IO.println <| Json.compress <| Json.mkObj [
        ("schema", "pj-tranche-b-prime-compiled-declaration-v1"),
...
end PJTrancheBPrimeDeclarationDump
