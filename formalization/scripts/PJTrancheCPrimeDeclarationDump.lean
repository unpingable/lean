...
/-
  Compiled declaration census for the provisional PJ Tranche C-prime
  candidate. Output is canonical JSON Lines using the same normalization as
  the ratified PJ-A and B-prime censuses.
-/
...
namespace PJTrancheCPrimeDeclarationDump
...
def canonicalLevelParamName (index : Nat) : Name :=
  Name.mkNum `_pj_tranche_c_prime_universe index
  ...
  for moduleName in selectedModules do
    ...
    for (name, info) in declarations do
      ...
      IO.println <| Json.compress <| Json.mkObj [
        ("schema", "pj-tranche-c-prime-compiled-declaration-v1"),
...
end PJTrancheCPrimeDeclarationDump
