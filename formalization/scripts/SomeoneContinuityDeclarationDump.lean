...
/-
  ...
  names.  This makes the resulting manifest sensitive to declaration content
  rather than presentation-only binder spelling.
-/
...
def canonicalLevelParamName (index : Nat) : Name :=
  Name.mkNum `_someone_continuity_universe index
...
run_cmd do
  ...
  for moduleName in selectedModules do
    let some moduleIdx := env.getModuleIdx? moduleName
      | throwError "selected Someone/Continuity module is absent: {moduleName}"
    ...
    for (name, info) in declarations do
      ...
      IO.println <| Json.compress <| Json.mkObj [
        ("schema", "someone-continuity-compiled-declaration-v1"),
