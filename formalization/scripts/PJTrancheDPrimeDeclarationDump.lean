...
/-
  Compiled declaration census for the provisional PJ Tranche D-prime
  hostile-audit candidate. Output is canonical JSON Lines using the same
  normalization as the ratified PJ-A, B-prime, and C-prime censuses.
-/
...
namespace PJTrancheDPrimeDeclarationDump
...
def canonicalLevelParamName (index : Nat) : Name :=
  Name.mkNum `_pj_tranche_d_prime_universe index
  ...
  for moduleName in selectedModules do
    ...
    for (name, info) in declarations do
      ...
      IO.println <| Json.compress <| Json.mkObj [
        ("schema", "pj-tranche-d-prime-compiled-declaration-v1"),
...
end PJTrancheDPrimeDeclarationDump
