/-
  ...
  across the zoo:
    ...
    3. discipline unsatisfiability (summary-as-authority -- CAGED HERE: the
                                 ...
                                 protected class; the four ANNEX skeleton
                                 replays below reuse this catch)
    4. MasterFree refutation    (universal crossroads -- CAGED HERE)
    5. wall replay (underivability) -- the four ANNEX walls replayed through
  ...
  | checkpoint-as-discharge      | CheckpointSettlement.checkpoint_cannot_discharge_unknown_commit (ANNEX; skeleton replay CAGED HERE) |
...
/-! ## Cage: ticket-spent-as-success (ANNEX wall replay)
    ...
    watchlist's ticket-spent-as-success entry; the ANNEX wall
    ...
    record funds nothing; the outcome needs its own receipt). The ANNEX wall
    proves more (stage semantics, the refused-commit witness); this cage is
    regression mass for the skeleton, not a substitute for the ANNEX theorem.
...
/-- **The wall replay** (the ANNEX statement's skeleton form): from the
...
/-! ## Cage: commit-attempted-as-executed (ANNEX wall replay)
    ...
    commit-attempted-as-executed entry; the ANNEX wall
    ...
    CAVEATS: as for the ticket cage -- custody shape only; the ANNEX wall's
...
/-! ## Cage: checkpoint-as-discharge (ANNEX wall replay)
    ...
    the watchlist's checkpoint-as-discharge entry; the ANNEX wall
    ...
    CAVEATS: the ANNEX wall additionally proves multiset conservation and
...
/-! ## Cage: observation-as-safety (ANNEX wall replay)
