/-
  ...
  Historical campaign note: landing this file did not itself tag v4. The
  operator later released the soaked crossing discipline; v13 corrects its
  repository path and custody record without changing the definitions.
...
import LeanProofs.CustodyIndexed.BridgeCompositionSequent

namespace LeanProofs.CustodyIndexed.CustodyIndexedSequent
...
open LeanProofs.CustodyIndexed.BridgeCompositionSequent (Judgment Calc)
...
theorem s4_entail_iff {Γ : List Judgment} {j : Judgment} :
  ...
  · intro h
    induction h with
    | ax hmem =>
        exact LeanProofs.CustodyIndexed.BridgeCompositionSequent.Entail.ax hmem
    | cut r _ _ ihs ihe =>
        cases r with
        | ts =>
            exact LeanProofs.CustodyIndexed.BridgeCompositionSequent.Entail.bridgeCutTS
              ihs ihe
        | sb =>
            exact LeanProofs.CustodyIndexed.BridgeCompositionSequent.Entail.bridgeCutSB
              ihs ihe
...
theorem s4_mint_provenance_via_generic
    {Γ : List Judgment}
    {b : LeanProofs.CustodyIndexed.BridgeCompositionSequent.DemoBoundary}
    {e : LeanProofs.CustodyIndexed.BridgeCompositionSequent.DemoExposure}
    (h : LeanProofs.CustodyIndexed.BridgeCompositionSequent.Entail Γ
      (Judgment.mMint b e)) :
    ∃ es : List Judgment,
...
theorem s4_master_free : MasterFree s4System := by
      ...
      have hsubSurface : Substantive s4System Calc.surface :=
        ⟨_, _, _,
          S4Rule.ts (env := LeanProofs.BoundedCalculi.TemporalCustody.permissiveEnv)
            ...
            (s := LeanProofs.CustodyIndexed.TemporalToSurfaceBridgeWiring.projectedSurface)
            (p := LeanProofs.CustodyIndexed.TemporalToSurfaceBridgeWiring.retainedProjectedUseProjection)
  ...
  | surface =>
      ...
      have hsubBoundary : Substantive s4System Calc.boundary :=
        ⟨_, _, _,
          S4Rule.sb (s := LeanProofs.CustodyIndexed.TemporalToSurfaceBridgeWiring.projectedSurface)
            (p := LeanProofs.CustodyIndexed.TemporalToSurfaceBridgeWiring.retainedProjectedUseProjection)
            (u := LeanProofs.BoundedCalculi.SurfaceProjection.Use.projectedUse)
            (b := LeanProofs.CustodyIndexed.BridgeCompositionSequent.openBoundary)
            (e := LeanProofs.CustodyIndexed.TemporalToSurfaceBridgeWiring.escapedExposure),
          Or.inr rfl⟩
...
end LeanProofs.CustodyIndexed.CustodyIndexedSequent
