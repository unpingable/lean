/-
  LeanProofs.CustodyIndexed.LinearNormalization -- v5 slice 2: linear structural
...
import LeanProofs.CustodyIndexed.StructuralNormalization

namespace LeanProofs.CustodyIndexed.LinearNormalization
...
def _root_.LeanProofs.CustodyIndexed.StructuralNormalization.Core.readsOf
...
/-- **Linearize**: pay every read with a distinct occurrence, threading
    ...
    of the read. -/
def _root_.LeanProofs.CustodyIndexed.StructuralNormalization.Core.linearize
...
theorem chainOf_linearize [DecidableEq J]
  ...
  | ax _ =>
      ...
      | some p =>
          simp only [Core.linearize, hrf] at h
          cases h
          simp [LeanProofs.CustodyIndexed.DerivationData.Deriv.chainOf,
            LeanProofs.CustodyIndexed.StructuralNormalization.Core.chainOf]
  | cut r ds de ih1 ih2 =>
      intro Δ Δ' d' h
      simp only [Core.linearize] at h
      cases h1 : ds.linearize Δ with
  ...
  | derive s d ih =>
      ...
      | ok Δ₁ d₁ =>
          ...
          cases h
          simp [Deriv.chainOf,
            LeanProofs.CustodyIndexed.StructuralNormalization.Core.chainOf, ih h1]
...
theorem cartesian_statable_but_linearly_refused :
    ...
    (dupTree.linearize [PJ.res]).isOk = false :=
  ⟨⟨LeanProofs.CustodyIndexed.StructuralNormalization.Core.toEntail dupTree⟩,
    linear_free_contraction_cannot_normalize⟩
...
end LeanProofs.CustodyIndexed.LinearNormalization
