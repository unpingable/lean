/-
  ...
  NoSilentProjection — terminal anti-laundering evidence.

  Opened as a scratch candidate on 2026-06-05 and adjudicated as terminal
  public evidence in v13. It is not part of an exact stable surface and does
  not discharge runtime conformance or a broader doctrine by itself.

  Provenance: drawn from three already-named
  ...
  Filing category (per the doctrine map's Lean filing discipline):
    Public proof-of-encodability evidence. It pays its rent by forcing the
    obligation lattice into a typed-Lean shape where the negative theorem
    stack is type-level checkable, without entering the stable API.
...
namespace Admissibility.NoSilentProjection
...
/-- Family-scoped conversion rule. `Converts source target a' a`
    ...
    INTENTIONALLY EMPTY (no constructors) in this evidence model — the
...
/-! ## Falsifiability witness — strip-and-restore log
    ...
    *two* constructors needed to fully refute the theorem (not just one
    ...
    output from `lake build LeanProofs.ViewSemantics.Evidence.NoSilentProjection`:
    ...
    form. `lake build LeanProofs.ViewSemantics.Evidence.NoSilentProjection` re-run
    confirmed green.
...
end Admissibility.NoSilentProjection
