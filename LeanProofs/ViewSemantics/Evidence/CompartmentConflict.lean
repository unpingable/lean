/-
  LeanProofs.ViewSemantics.Evidence.CompartmentConflict -- the HAL obstruction: no safe
  ...
  (closed countermodels; no `sorry`, no `Classical`).
  Candidate note:
    ~/git/papers/working/compartmentalization-distinguishability-axis.md
  Sibling countermodel (view too FINE): `Evidence/MosaicRelease.lean`. This file
  ...
  THE STATEMENT:
    ...
    * `disclosed_view_supports_total_safe_policy` +
      ...
      payload remains ambiguous in every disclosed-view fiber. This fixture
      now instantiates the shared bounded-sufficient-projection core; the
      separate Gate B resident-ontology adapter adjudicates the obligation
      question as an orthogonal view-context axis rather than making that
      claim here.
...
/-- Compatibility alias for the shared view-semantics core. -/
abbrev Determines {W O S : Type} (view : W → O) (f : W → S) : Prop :=
  LeanProofs.ViewSemantics.Determines view f

/-- The local duty is exactly the deterministic task generated by the hidden
discriminator. -/
theorem matchSafe_eq_requiredSafe :
    MatchSafe = LeanProofs.ViewSemantics.RequiredSafe discriminator :=
  rfl

/-- The blind observation is genuinely insufficient for the total duty, not
merely for the two-world spelling of the HAL obstruction. -/
theorem blind_view_not_operationally_sufficient :
    ¬ LeanProofs.ViewSemantics.OperationallySufficient blindView MatchSafe := by
  rintro ⟨policy, hsafe⟩
  exact blind_actor_has_no_safe_total_policy
    ⟨policy, hsafe (false, false), hsafe (true, false)⟩

/-- The disclosed discriminator is both no finer than its declared budget
and sufficient for the duty. -/
theorem disclosed_view_is_bounded_sufficient :
    LeanProofs.ViewSemantics.BoundedSufficient
      discView discView MatchSafe :=
  ⟨LeanProofs.ViewSemantics.refines_refl discView,
   disclosed_view_supports_total_safe_policy⟩

/-- Every discriminator observation leaves both payload values possible. -/
theorem disclosed_view_payload_fiberwise_ambiguous :
    LeanProofs.ViewSemantics.FiberwiseAmbiguous discView payload := by
  intro w
  rcases w with ⟨d, p⟩
  refine ⟨(d, !p), rfl, ?_⟩
  cases p with
...
the private payload ambiguous in every fiber.  The legacy weak receipt remains
as a corollary for compatibility. -/
theorem disclosed_view_does_not_determine_payload :
    ¬ Determines discView payload :=
  LeanProofs.ViewSemantics.fiberwiseAmbiguous_notFullyDetermining
    ⟨(false, false)⟩ disclosed_view_payload_fiberwise_ambiguous
