/-
  Wired.Coordinates — the interval/order coordinate.
  ...
  soundness here consumes ONLY transitivity (`btrans`) — see `Wired.CarryLaws`
  for why that is exactly the cost.
-/
import Wired.CarryLaws

namespace Wired.Coordinates
