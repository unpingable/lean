/-
  Wired.Freshness — the assembled freshness kernel (double-entry transport).
  ...
  WIRED: the window receipt is discharged by `Wired.Coordinates`
  (transitivity), the divergence receipt by `Wired.Divergence` (triangle+budget).
...
namespace Wired.Freshness

open Wired.Coordinates (Window ValidAt Transport transport_sound_under_transitive_relation)
open Wired.Divergence (divergence_transport_widens)
