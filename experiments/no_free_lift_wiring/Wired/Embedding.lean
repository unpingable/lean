/-
  Wired.Embedding — THE WIRE. Authority ⊕ Freshness embedded into the spine.
...
namespace Wired.Embedding

-- (sibling namespaces `NoFreeLift`, `Authority`, `Freshness`, `Coordinates`,
-- `Divergence` resolve via the enclosing `Wired` namespace — no `open` needed.)

/-- An authority claim: a verdict triple. -/
structure AuthClaim where
  b : Authority.BasisVerdict
  p : Authority.PrecedenceVerdict
  s : Authority.StandingVerdict
...
def Sem : Claim → Prop
  | Sum.inl a => Authority.authorityVerdict a.b a.p a.s = Authority.AuthorityVerdict.authorized
...
theorem cross_axis_unpaid_unsound :
  ...
  · exact Authority.green_triple_authorizes
  ...
  · exact CrossNaked.crossJump
      (CrossNaked.base (c := Sum.inl ⟨.admissibleBasis, .resolved, .standing⟩)
        Authority.green_triple_authorizes)
