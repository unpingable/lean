/-
  Wired.Authority — the authority verdict kernel.

  Custody class: MODELED KERNEL (a study copy of the authority verdict algebra;
  NOT the canonical `Admissibility/Authority.lean`). A NO-BRIDGE kernel: it
  admits locally and exposes no transport, so in the customs office it is
  bridge-inert (see `Wired.Embedding.authority_is_conservative`).
-/

namespace Wired.Authority
