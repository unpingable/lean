/-
  Successor.WitnessedDerivation — the judgment layer of the witnessed-derivation
  calculus (1.3 candidate).

  Custody class: EXPERIMENTAL-WIRING. NOT in `defaultTargets`; build with
  `lake build Successor`. The `Wired` spine is untouched — this lib only imports it.
...
namespace Successor.WitnessedDerivation

open Wired.NoFreeLift
...
end Successor.WitnessedDerivation
